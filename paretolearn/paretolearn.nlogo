...
globals [cooperate defect]

breed [agents agent]

;n: two-dimensional list that contains the number of times whe have played each pair of actions
;    item my-action (item his-action n)
;alpha is the learning rate
;action: the action the agent takes
agents-own [n n-joint alpha epsilon action]
...
to setup
 ...
 ; set payoffs [[3 0][5 1]] ;action 0 is cooperate, 1 is defect
  ...
  create-agents 2 [
    ...
    set n [[1 1][1 1]] ;start all at 1 to avoid division by zero
  ]
  ask (agent 0) [
    set xcor -5
  ]
...
to go
  ...
  ask agents [
   ; set alpha learning-rate ;in case the user changes it
    adapt
   ]
  ask (agent 0) [
    set-current-plot "Expected Utility"
    let eucooperate get-expected-utility cooperate
    set-current-plot-pen "cooperation"
    plot eucooperate
    set-current-plot-pen "defection"
    let eudefect get-expected-utility defect
    plot eudefect
    set-current-plot "Actions"
    plotxy (ni defect - ni cooperate) ([ni cooperate - ni defect] of (agent 1))
    set xcor 0 - (max-pxcor * eudefect / (eucooperate + eudefect))
  ]
  ask (agent 1) [
    let eucooperate get-expected-utility cooperate
    let eudefect get-expected-utility defect
    set xcor max-pxcor * eudefect / (eucooperate + eudefect)
  ]
end
...
to-report get-expected-utility [my-action]
  report sum map [(utility my-action ?) * ( (nij my-action ?) / (ni my-action))] [0 1]
end
...
to take-action
  ifelse (ticks < 400) [ ;initial exploration rate of 400
    set action random 2
  ][
    ;Following Definition 1 from the paper: "A JCAL learner is an agent i who at time t chooses an action a with a probability propotional to the expected utility"
    let Evector (map [get-expected-utility ?] [0 1])
    ;set action stochastic-choice Evector
    
    ;Section 3.2 of the paper contradicts Definition 1 and says instead that the action chosen, with probability 1-epsilon, is the one that maximizes the agent's utility
    ; and with probability epsilon whe choose the other action
  ...
  ifelse (action = 0) [
    set color green ;cooperate
  ][
    set color red ;defect
  ]
end

to adapt
  let his-action (first [action] of other agents)
  update-count action his-action
;  let q-value qij action his-action
;  update-q action his-action (q-value + alpha * ((utility action his-action) - q-value)) ;eq (6)
end
@#$#@#$#@
GRAPHICS-WINDOW
207
