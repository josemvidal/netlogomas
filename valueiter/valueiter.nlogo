;a node turtle sits in the middle of each patch
;each node is also a state in the mdp
breed [nodes node]

;each link represents an action
;transitions: [[.8 nodea] [.2 nodeb]]
;even thought a link only points to one other node, the transions variable actually holds all the 
;u: utility, new-u, the new one for t+1
links-own [u new-u transitions]

;reward: fixed reward for reaching state
;utility: expected utility, calculated by the value iteration algorithm
nodes-own [reward utility new-utility]
...
Line -7500403 true 150 0 120 30
Line -7500403 true 150 0 180 30
