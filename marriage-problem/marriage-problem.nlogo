...
to setup
  ...
  create-women 64 [
    ...
    set size 1.5
    setxy 3 + (((max-pxcor) / 8) * (who mod 8)) 3 + ((max-pycor) / 8) * (floor (who / 8))
  ]
...
;Make it look pretty. Men move around the woman they point to.
to layout
  no-display
  ask women [
    let old-x xcor
    let old-y ycor
    layout-radial in-link-neighbors links self
...
to go
  ...
  ifelse (ticks mod 2 = 0) [
    ...
    ask women with [any? in-link-neighbors] [
      let suitor one-of in-link-neighbors
      set label 1 + position suitor preferences
    ]
    ask men [
      set label 64 - length preferences + 1
    ]
    set-current-plot "Matchings by Prefence"
    clear-plot 
    ask links [
      plotxy ([label] of end1) ([label] of end2)
    ]
    set-current-plot "Bachelors"
    plot count men with [not any? my-out-links]
...
to turn-down-proposals
  let suitors sort-by [position ?1 preferences < position ?2 preferences] in-link-neighbors
  if (length suitors <= 1) [stop]
  foreach (butfirst suitors) [
    ask ? [
