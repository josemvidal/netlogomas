...
;preferences - list of agents in order of preference: (first preferences) is the agent that owns the house I like best, and so on.
;reach-me-agents - list of who of agents that can reach me
;color - blue agents are in the game, green agents have already found a trade (cycle).
agents-own [preferences reach-me-agents]

;cycle-steps - number of steps left before we end the cycle. When it reaches 0 agents that are still blue will re-align their edges to point to another blue agent.
globals [cycle-steps]
...
to setup
  ...
  let all-agents sort agents
  ask agents [
    set preferences shuffle all-agents
  ]
  ask agents [
     setup-link
  ]
  repeat 50 [
    layout-spring agents links 0.2 5 1]
  set cycle-steps count agents
end

to go
  if (cycle-steps = 0) [stop] ;all done
  tick
  layout
  ask agents [
    send-message ]
  ask agents [ ;if I can reach myself, I drop out
    if (member? who reach-me-agents) [
      set color green
      set label num-agents - length preferences + 1
    ]
  ]
  set cycle-steps (cycle-steps - 1)
  if (cycle-steps = 0) [;end of cycle, time for agents to re-set their most preferred
    ask links with [[color] of end2 = green][ ;links that point to green agents are set to black
      set color black
    ]
    histogram [num-agents - length preferences + 1] of (agents with [color = green])
    set cycle-steps count (agents with [color = blue])
    ask agents with [color = blue][
      setup-link
    ]    
  ]
end
...
to layout
  layout-spring agents links 0.2 5 .2
  if mouse-down? [
...
;agent functions
;
;make link to my most prefered blue agent, except if that agent is me.
to setup-link
  set reach-me-agents []
  ask my-out-links [die]
  while [[color] of (first preferences) = green] [ ;set my first preference to the first one that is not green (still in the game)
    set preferences butfirst preferences
  ]
  if (first preferences != self) [
...
to send-message
  ifelse (any? out-link-neighbors) [
    if (count out-link-neighbors != 1) [   show "ERROR: there should only be one outlink"  ] ;assert out-link-neighbors == 1
    let him one-of out-link-neighbors
    ask him [
      set reach-me-agents union (fput ([who] of myself) ([reach-me-agents] of myself)) reach-me-agents
    ]
;    set ([reach-me-agents] of him) union (fput who reach-me-agents) ([reach-me-agents] of him)
  ][ ; I link to myself
    set reach-me-agents (list who)
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
